...
class StoreProductRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        $rules = [
            ...
            'art_code' => ['required', 'string', 'max:255', 'unique:products'],
            'product_no' => ['required', 'string', 'unique:products'],
        ];

        if ($this->has('has_variants') && $this->has_variants === 'active') {
            // Validation for variants
            $rules['variants_sku'] = ['required', 'array'];
            $rules['variants_sku.*'] = ['required', 'string', 'distinct', Rule::unique('product_variants', 'sku')];
            ...
            $rules['product_sku_single'] = ['required', 'string', Rule::unique('product_variants', 'sku')];
