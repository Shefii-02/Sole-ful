    ...
    $(document).ready(function() {
        ...
        function getVariants(opt, idx, prefix = null) {
            ...
            (opt[idx].values).forEach(function(item) {
                ...
                if (opt[idx + 1] != null) {
                    getVariants(opt, idx + 1, prefix + (idx === 0 ? '' : ',') + opt[idx].option + ':' +
                        item);
                } else {
                    variant_options.push(prefix + (idx === 0 ? '' : ',') + opt[idx].option + ':' +
                        item);
                }
        ...
        $("body").on("change blur", ".option-select", function() {
            ...
            if ($(this).val() == 'new-option') {
                ...
                $(this).parent().parent().find('.option-name').val($.trim($(this).find("option:selected")
                    .text())).trigger('change')

                var skuFiled = $(this).parent().parent().find('.option-sku');
                const selValues = $(this).find("option:selected").val();
                const artCode = $("#art_code").val();
                const productNo = $("#product_no").val();
                if (selValues != '') {
                    $.ajax({
                        url: "{{ route('admin.products.generate-autosku') }}",
                        method: "GET",
                        data: {
                            'selValues': selValues,
                            'art_code': artCode,
                        ...
                        async: false, // Ensures synchronous behavior (not recommended for production)
                        success: function(response) {
                            skuFiled.val(response).trigger('change');
                        },
                        error: function(xhr, status, error) {
                            validation_errors.push(
                                "Error occurred while SKU auto generate" + error
                                );
                        },
                    });
                }
        ...
        $("#save-options").click(function(e) {
            ...
            $(".option-type").each(function() {
                ...
                if (checker[selection]) {
                    ...
                    showError("Oops...", "<h4>Sorry, please fix...</h4>" +
                        "<p style='color:red;'>Option type can't be used twice!</p>");
                    duplicated = true;
                    return false;
                } else {
                ...
                $(this).parent().parent().find(".option-value").each(function() {
                    ...
                    if (xchecker[xselection]) {
                        //if the property is defined, then we've already encountered this value
                        showError("Oops...", "<h4>Sorry, please fix...</h4>" +
                            "<p style='color:red;'>Duplicate values are not allowed!</p>"
                        );
                        duplicated = true;
                        return false;
                    } else {
            ...
            $(`<div class="col-md-6 mb-3">
                <div class="box-div">
                    <div class="row position-relative">
                        ...
                        <div class="col-md-6">
                            <select class="form-control option-type option-type-` + rand_name + `" data-random="` +
                rand_name +
                `" name="variant_options[` + rand_name + `]">
                        ...
                        <div class="col-md-5 option-value-container" data-random="` + rand_name + `">
                            <select data-random="` + rand_name + `" name="variant_option_values[` + rand_name +
                `][]" class="form-control option-value option-value-` + rand_name + ` mb-2">
                            ` + sizeOptions + `
                            </select>
        ...
        $("body").delegate(".add-option-value", "click", function(e) {
            ...
            <select data-random="` + KeyVal + `" name="variant_option_values[` + KeyVal +
                    `][]" class="form-control option-value  option-value-` + KeyVal + ` mb-2">
            ` + content + `
        ...
        $("body").delegate(".option-value", "change", function() {
            ...
            $(this).parent().find(".option-value").each(function() {
                ...
                if (checker[selection]) {
                    //if the property is defined, then we've already encountered this value
                    showError("Oops...", "<h4>Sorry, please fix...</h4>" +
                        "<p style='color:red;'>Option value duplication not allowed!</p>");
                    $(this).val('')
                    duplicated = true;
                    return false;
                } else {
        ...
        $(".add-variant").click(function(e) {
            ...
            var instore_show = $("#platform-instore").is(":checked") ? 'style="display:inline-block;"' :
                'style="display:none;"';
            var online_show = $("#platform-online").is(":checked") ? 'style="display:inline-block;"' :
                'style="display:none;"';
            var rand_name = Math.random().toString(36).substring(2, 9);
        ...
        <div class="col-md-12">
            <div class="row productVariant" data-id="${rand_name}">
                <div class="col-md-2">
                    <select class="form-control option-select" name="variants[` + rand_name + `]" data-option="` +
                rand_name + `"><option value="">Select an Option</option>
        ...
        function getOptionSelection() {
            ...
            variant_options.forEach(function(item) {
                ...
                options_select = options_select + '<option value="' + item + '">' + line.trim() +
                    '</option>' + "\n";
            })
            ...
            <div class="box-div">
                <div class="row">
                    <div class="col-md-7">
                        ...
                        <input type="file"  class="product-image" data-img="img-` + rand_name + `"  data-src="src-` +
                rand_name + `" />
                        <img src="/assets/img/noimage.png" style="width:200px;height:auto;" class="mt-3" id="img-` +
                rand_name + `" />
        ...
        $("body").delegate(".del-image", "click", function(e) {
            e.preventDefault();
            $(this).parent().parent().parent().parent().remove();
            $("#product-images").append(`<input type="hidden" name="deleted_images[]" value="` + $(this)
                .attr("data-id") + `" />`);
        })
        ...
        //         $.ajax({
        //             url: "{{ route('admin.products.art_code-check') }}",
        ...
        //                     validation_errors = response.message ||  "ART code already exists.";
        //                 } else {
        //                     // console.log("ART code is available.");
        //                 }
        //             },
        //             error: function(xhr, status, error) {
        //                 validation_errors = "Error occurred: " + error;
        //             }
        //         });
        ...
        //         $.ajax({
        //             url: "{{ route('admin.products.product_no-check') }}",
        //             method: "GET", // Or POST if required
        //             data: {
        ...
        //             success: function(response) {
        //                 if (response.status === 208) {
        //                     validation_errors = response.message ||  "Product number already exists.";
        //                     alert( validation_errors  ); // Replace with your preferred method to show errors
        //                 } else {
        //                     // console.log("Product number is available.");
        //                 }
        //             },
        //             error: function(xhr, status, error) {
        //                 validation_errors = "Error occurred: " + error;
        ...
        //         showError('Oops...', "<h4>Sorry, please fix...</h4>" + "<p style='color:red;'>" +
        //             validation_errors + '</p>');
        //         return false;
        //     }
        ...
        $("#save-product-info, #save-product-info").click(function(e) {
            ...
            // Validate ART Code
            if ($("#art_code").val() === "") {
                validation_errors.push("A product ART code is required!");
            } else {
                const art_code = $("#art_code").val();
                var product_id = `{{ isset($product) ? $product->id : '' }}`;
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.status === 208) {
                            validation_errors.push(response.message ||
                                "ART code already exists.");
                        }
                    },
                    error: function(xhr, status, error) {
                        validation_errors.push("Error occurred while checking ART code: " +
                            error);
                    },
                });
            }
            ...
            if ($("#product_no").val() === "") {
                validation_errors.push("A product number is required!");
            } else {
                const product_no = $("#product_no").val();
                var product_id = `{{ isset($product) ? $product->id : '' }}`;
                ...
                $.ajax({
                    ...
                    error: function(xhr, status, error) {
                        validation_errors.push(
                            "Error occurred while checking product number: " + error);
                    },
                });
            }
            ...
            if ($("#product_name").val() === "") {
                validation_errors.push("A product name is required!");
            }

            // Display Errors if Found
            if (validation_errors.length > 0) {
                const errorHtml = `
            <h4>Sorry, please fix...</h4>
            <ul style="color: red;">
                ${validation_errors.map((err) => `<li>${err}</li>`).join("")}
            </ul>
        `;
                showError("Oops...", errorHtml);
                return false;
            }
        ...
        $("#save-product-variants, #save-product-variants").click(function(e) {
            ...
            const addError = (error) => validationErrors.push(error);
            ...
            if (!$("#has-variants").is(":checked")) {
                ...
                if (!$("input[name=product_sku_single]").val().match(pattern)) {
                    validation_errors +=
                        'SKU must be Alphanumeric, no space or special characters allowed' + "<br/>";
                } else if ($("input[name=product_sku_single]").val() != '') {
                    // //check value already exist our db
                    // $.ajax({
                    //     url: "{{ route('admin.products.sku-check') }}",
                    //     cache: false,
                    //     data: {
                    //         id: menuId
                    //     },
                ...
                const singleSku = $("input[name=product_sku_single]").val();
                const singlePrice = $("input[name=product_price_single]").val();
                var product_id = `{{ isset($product) ? $product->id : '' }}`;

                // Check SKU availability via AJAX
                if (singleSku) {
                    $.ajax({
                        url: "{{ route('admin.products.sku-check') }}",
                        method: "GET",
                        ...
                        async: false,
                        success: function(response) {
                            if (response.status === 208) addError(response.message ||
                                "SKU already exists.");
                        },
                        error: function() {
                            addError("Error occurred while checking SKU availability.");
                        },
                    });
                }

                // if (!singlePrice) addError("A price value is required.");
                // if (singlePrice && !/^\d+(\.\d{1,2})?$/.test(singlePrice)) addError(
                //     "Price must be numeric (e.g., 1.00).");
                ...
                $(".option-sku").each(function(item) {
                    ...
                    var obj = {
                        'id': id,
                        'option': $(this).parent().parent().find('.option-select').val(),
                        'sku': $(this).parent().parent().find('.option-sku').val(),
                        'name': $(this).parent().parent().find('.option-name').val()
                    }
                ...
                $(".option-select").each(function() {
                    ...
                    if ($(this).val() == '' || $(this).val() == 'new-option') {
                        validation_errors += 'All variants must have an option selected' +
                            "<br/>";
                        return false;
                    }
                    ...
                    if (checker[selection]) {
                        //if the property is defined, then we've already encountered this value
                        validation_errors += $(this).find("option:selected").text() +
                            " can't be used twice!" + "<br/>";
                        return false;
                    } else {
                        //first time we've seen this value, so let's define a property for it
                        checker[selection] = true;
                ...
                $(".option-sku").each(function() {
                    ...
                    if (!$(this).val().match(pattern)) {
                        validation_errors +=
                            'SKU must be Alphanumeric, no space or special characters allowed' +
                            "<br/>";
                        return false;
                    } else {
                    ...
                    if (checker[selection]) {
                        //if the property is defined, then we've already encountered this value
                        validation_errors += 'SKU ' + $(this).val() + " can't be used twice!" +
                            "<br/>";
                        return false;
                    } else {
                        //first time we've seen this value, so let's define a property for it
                ...
                $(".option-sku").each(function() {
                    const sku = $(this).val();
                    var product_id = `{{ isset($product) ? $product->id : '' }}`;
                    if (!sku) return addError("All variants must have a SKU.");
                    // if (!isSkuValid(sku)) return addError(
                    //     "SKU must be alphanumeric, no spaces or special characters allowed."
                    // );

                    // Check SKU uniqueness
                    // if (variantChecker[sku]) {
                    //     return addError(`SKU '${sku}' can't be used twice.`);
                    // }
                    // variantChecker[sku] = true;
                    ...
                    $.ajax({
                        ...
                        error: function() {
                            addError(
                                `Error occurred while checking SKU availability for '${sku}'.`
                            );
                        },
                    });
                });
                ...
                $(".option-name").each(function() {
                    ...
                    if (checker[selection]) {
                        //if the property is defined, then we've already encountered this value
                        validation_errors += 'Variant name ' + $(this).val() +
                            " can't be used twice!" + "<br/>";
                        return false;
                    } else {
                        //first time we've seen this value, so let's define a property for it
        ...
        //     // Helper function for checking SKU format
        //     const isSkuValid = (sku) => /^[A-Z0-9]+$/i.test(sku);

        //     // Validate Single Product (No Variants)
        //     if (!$("#has-variants").is(":checked")) {
        //         const singleSku = $("input[name=product_sku_single]").val();
        //         const singlePrice = $("input[name=product_price_single]").val();

        //         if (!singleSku) addError("A SKU (Stock Keeping Unit) is required.");
        //         if (singleSku && !isSkuValid(singleSku)) addError(
        //             "SKU must be alphanumeric, no spaces or special characters allowed.");
        ...
        //                 async: false,
        //                 success: function(response) {
        //                     if (response.status === 208) addError(response
        ...
        //         $(".option-name").each(function() {
        //             const name = $(this).val();
        //             if (!name) addError("All variants must have a name.");
        //         });

        //         $(".option-price").each(function() {
        //             const price = $(this).val();
        //             if (!price) return addError("All variants must have a price.");
        //             if (!/^\d+(\.\d{1,2})?$/.test(price)) return addError(
        //                 "All variant prices must be numeric (e.g., 1.00).");
        //         });
        //     }
