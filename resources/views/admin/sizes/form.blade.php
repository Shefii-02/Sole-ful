    ...
    <div
        ...
        <nav>
            <ol class="flex flex-wrap items-center gap-2">
                ...
                <li>
                    <a class="flex items-center gap-3 font-medium" href="{{ route('admin.sizes.index') }}">
                      ...
                      <span class="hover:text-primary">Sizes</span>
                    </a>
                  </li>
        ...
        <div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
            <div class="border-b border-stroke px-6.5 py-4 dark:border-strokedark">
                <h2 class="text-title-sm2 font-bold text-black dark:text-black">
                    {{ isset($size) ? 'Edit sizes' : 'Create a sizes' }}
                </h2>
            </div>
            ...
            <form enctype="multipart/form-data"
                ...
                <div class="p-6.5 ">
                    ...
                    <div class="mb-2 w-1/2">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Size Name</label>
                        <input type="text" autocomplete="off" name="size_value" class="form-control" value="{{ old('size_value', $size->size_value ?? '') }}" />
                        @error('size_value')
                    ...
                    <div class="mb-2 w-1/2">
                        <label class="block mb-2 text-sm font-medium text-black dark:text-dark">Size Code</label>
                        <input type="text" autocomplete="off" name="size_code" class="form-control" value="{{ old('size_code', $size->size_code ?? '') }}" />
                        @error('size_code')
                            <span class="fw-bold text-danger">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-2 w-1/4">
                        ...
                        <select class="form-control mt-2" name="display_order" required>
                            @for ($i = 1; $i < 20; $i++)
                                <option {{ isset($size) && $size->display_order == $i ? 'selected' : '' }}
                                    value="{{ $i }}">{{ $i }}</option>
                            @endfor
                        </select>
                        @error('display_order')
