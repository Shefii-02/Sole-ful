      ...
      <table cellpadding="10" cellspacing="0" align="center" border="0" style="border:none;" >
          <tr>
              ...
              <td align="center" style="padding: 10px 15px;">
                  <big><strong>Invoice#  <br/><span style="color:#e480b0;">{{$order_details->invoice_id}}</span></strong></big>
              </td>
          </tr>
          <tr  align="center">
              <td align="center" colspan="2" style="padding: 10px 15px;border-top:1px solid #DDD;">
                  <big><strong>Order Amount <br/><span style="color:#e480b0;">{{getPrice($order_details->grandtotal)}}</span></strong></big>
              </td>
            ...
            <tr>
                ...
                @else
                    ...
                    @if($pickupAddress)
                        ...
                        <small><small><strong><a target="_new" href="{{url('/stores/'.$pickupAddress->slug)}}" style="color:#e480b0;text-decortion:none;">VIEW LOCATION</a></strong></small></small><br/>
                        -------<br/>
         ...
         <table style="width: 100%;" cellpadding="7" cellspacing="0" border="0">
            ...
            <tr>
               <td style="border-bottom: 1px solid #DDD;">
                      ...
                      @if($items_listing->bundle_product == '1')
                        <table>
                        @foreach($items_listing->bundle_items as $itm)
                        <tr>
                            <td>
                                <div class="fw-normal p-0" style="padding: 2px 4px !important;!I;!;border: 1px solid #d7d7d7;border-radius:2px;font-size:12px;margin-bottom: 2px;">{{ $itm->product_name }}</div>
                            </td>
                        </tr>
                        @endforeach
                        </table>
                       ...
                       <br>
                        ...
                        <div class="d-flex">
                            ...
                            <small class="text-theme fw-bold" style="color:#f591c1;font-weight:800">{{ getPrice($items_listing->price_amount) }}</small>
                            
                            @if($items_listing->product->has_special_price == 1)
                            
                                @php
                                
                                if($items_listing->product->discount_type == 'percentage')
                                {
                                    if($items_listing->actual_price  != 0){
                                        $discountPercentage = (($items_listing->actual_price - $items_listing->price_amount) / $items_listing->actual_price * 100);
                                    }
                                    else{
                                    ...
                                    if ($firstDecimal > 0) {
                                        $discountType = getPrice($items_listing->actual_price - $items_listing->price_amount) . " OFF";
                                    } else {
                                ...
                                @endphp
                               <small class="text-body-tertiary me-1"> <strike>({{getPrice($items_listing->actual_price)}})</strike> <span  style="color:#FF0000">{{$discountType}}</span></small>
                            @endif
                        </div>
                   </td>
               ...
               <td style="border-bottom: 1px solid #DDD;" align="right" valign="middle">
                ...
                @if(($parentItem = $items_listing->parentItem) && $items_listing->bundle_product != '1')
                    @foreach($parentItem as $item_list)
                        <tr class="products-checkout col-12">
                           <td  style="border-bottom: 1px solid #DDD;" valign="middle">
                              
                              {{$item_list->product_name}}<br>{{$item_list->product_sku}} <br/>{{$item_list->variation}}
                           </td>
                           <td  style="border-bottom: 1px solid #DDD;" align="center" valign="middle">
                              {{$item_list->quantity}}
                           </td>
